...
# 3-steps lookahead agent with grid state representation to speed up the simulation

class MSLAGridAgent(AbstractAgent):
    ...
    def act(self, state):
        ...
        self.reward_for_actions = self.msla_rollout()
        # print("reward for actions:", self.reward_for_actions)
        return np.argmax(self.reward_for_actions)

    def msla_rollout(self, depth=0):
        state_0 = self.mdp.state
        reward_for_actions = np.zeros(self.mdp.transition.shape[1])
        for a_0 in range(self.mdp.transition.shape[1]):
            reward_0 = 0
            state_1 = self.mdp.transition[state_0, a_0]
            reward_for_actions[a_0] += self.config["gamma"] ** depth * self.mdp.reward[state_1, a_0]
            reward_for_actions_1 = reward_for_actions[a_0] * np.ones()
            for a_1 in range(self.mdp.transition.shape[1]):
                state_2 = self.mdp.transition[state1, a1]
                reward_for_actions[a1] += self.config["gamma"] ** (depth + 1) * self.mdp.reward[state1, a0]
                for a_2 in range(self.mdp.transition.shape[1]):
                    for step in range(depth, self.config["horizon"]):

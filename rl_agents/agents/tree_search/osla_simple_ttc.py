    ...
    def preference_policy(state, observation, action_index, ratio=2):
        ...
        for i in range(len(available_actions)):
            if available_actions[i] == action_index:
                ...
                return available_actions, probabilities
        return OSLATTCAgent.random_available_policy(state, observation)


class OSLATTC(AbstractPlanner):
    """
       An implementation of One Step Look Ahead.
    """
    def __init__(self, env, config=None):
        """
            New OSLATTC instance.

        :param config: the OSLATTC configuration. Use default if None.
        """
    ...
    @classmethod
    def default_config(cls):
        cfg = super(OSLATTC, cls).default_config()
        cfg.update({
            "temperature": 0,
            "closed_loop": False
        })
    ...
    def reset(self):
        self.root = OSLATTCNode(parent=None, planner=self)
    ...
    def step_by_prior(self, action):
        """
            Replace the OSLATTC tree by its subtree corresponding to the chosen action, but also convert the visit counts
    ...
    def get_child(self, action, observation=None):
        ...
        if observation is not None:
            if str(observation) not in child.children:
                child.children[str(observation)] = OSLATTCNode(parent=child, planner=self.planner, prior=0)

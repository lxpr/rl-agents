...
class OSLAAgent(AbstractTreeSearchAgent):
    """
        An agent that uses One Step Look Ahead to plan a sequence of action in an MDP.
    """
    def make_planner(self):
        prior_policy = OSLAAgent.policy_factory(self.config["prior_policy"])
        rollout_policy = OSLAAgent.policy_factory(self.config["rollout_policy"])
        return OSLA(self.env, prior_policy, rollout_policy, self.config)
    ...
    def default_config(cls):
        ...
        config.update({
            ...
            "env_preprocessors": [{
                ...
                "args": "highway_env.vehicle.behavior.IDMVehicle"
            }]
         })
        return config
    ...
    def policy_factory(policy_config):
        if policy_config["type"] == "random":
            return OSLAAgent.random_policy
        elif policy_config["type"] == "random_available":
            return OSLAAgent.random_available_policy
        elif policy_config["type"] == "preference":
            return partial(OSLAAgent.preference_policy,
    ...
    def preference_policy(state, observation, action_index, ratio=2):
        ...
        for i in range(len(available_actions)):
            if available_actions[i] == action_index:
                ...
                return available_actions, probabilities
        return OSLAAgent.random_available_policy(state, observation)


class OSLA(AbstractPlanner):
    """
    ...
    def __init__(self, env, prior_policy, rollout_policy, config=None):
        ...
        :param config: the OSLA configuration. Use default if None.
    ...
    @classmethod
    def default_config(cls):
        cfg = super(OSLA, cls).default_config()
        cfg.update({
    ...
    def reset(self):
        self.root = OSLANode(parent=None, planner=self)
    ...
    def step_by_prior(self, action):
        """
            Replace the OSLA tree by its subtree corresponding to the chosen action, but also convert the visit counts
...
class OSLANode(Node):
    ...
    def __init__(self, parent, planner, prior=1):
        super(OSLANode, self).__init__(parent, planner)
        self.value = 0
        self.prior = prior
    ...
    def get_child(self, action, observation=None):
        ...
        if observation is not None:
            if str(observation) not in child.children:
                child.children[str(observation)] = OSLANode(parent=child, planner=self.planner, prior=0)

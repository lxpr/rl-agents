...
class OSLAWithPriorPolicyAgent(AbstractTreeSearchAgent):
    """
        An agent that uses One Step Look Ahead to plan a sequence of action in an MDP.
    """
    def make_planner(self):
        prior_policy = OSLAWithPriorPolicyAgent.policy_factory(self.config["prior_policy"])
        rollout_policy = OSLAWithPriorPolicyAgent.policy_factory(self.config["rollout_policy"])
        return OSLAWithPriorPolicy(self.env, prior_policy, rollout_policy, self.config)
    ...
    def policy_factory(policy_config):
        if policy_config["type"] == "random":
            return OSLAWithPriorPolicyAgent.random_policy
        elif policy_config["type"] == "random_available":
            return OSLAWithPriorPolicyAgent.random_available_policy
        elif policy_config["type"] == "preference":
            return partial(OSLAWithPriorPolicyAgent.preference_policy,
                           action_index=policy_config["action"],
                           ratio=policy_config["ratio"])
        elif policy_config["type"] == "one_ahead":
            return OSLAWithPriorPolicyAgent.one_ahead_policy
        else:
    ...
    @staticmethod
    def one_ahead_policy(state, observation):
        """
            Choose actions from a uniform distribution.
    ...
    def preference_policy(state, observation, action_index, ratio=2):
        ...
        for i in range(len(available_actions)):
            if available_actions[i] == action_index:
                ...
                return available_actions, probabilities
        return OSLAWithPriorPolicyAgent.random_available_policy(state, observation)


class OSLAWithPriorPolicy(AbstractPlanner):
    """
    ...
    def __init__(self, env, prior_policy, rollout_policy, config=None):
        """
            New OSLAWithPriorPolicy instance.

        :param config: the OSLAWithPriorPolicy configuration. Use default if None.
        ...
        self.prior_agent = agent_factory(env, self.config['prior_agent'])
        #  Load the prior agent from a file, if one is set
        if 'model_save' in self.config['prior_agent']:
            self.prior_agent.load(self.config['prior_agent']['model_save'])
        self.env = env
    ...
    @classmethod
    def default_config(cls):
        cfg = super(OSLAWithPriorPolicy, cls).default_config()
        cfg.update({
    ...
    def reset(self):
        self.root = OSLAWithPriorPolicyNode(parent=None, planner=self)
    ...
    def run(self, state, observation, i):
        ...
        while depth < self.config['horizon'] and not terminal:
            ...
            if depth == 0:
                ...
                action = self.prior_agent.act(observation)
                print('prior agent action:', action)
            # print('action:', action)
            # print('depth:', depth)
    ...
    def step_by_prior(self, action):
        """
            Replace the OSLAWithPriorPolicy tree by its subtree corresponding to the chosen action, but also convert the visit counts
...
class OSLAWithPriorPolicyNode(Node):
    K = 1.0
    """ The value function first-order filter gain"""

    def __init__(self, parent, planner, prior=1):
        super(OSLAWithPriorPolicyNode, self).__init__(parent, planner)
        self.value = 0
        self.prior = prior
    ...
    def get_child(self, action, observation=None):
        ...
        if observation is not None:
            if str(observation) not in child.children:
                child.children[str(observation)] = OSLAWithPriorPolicyNode(parent=child, planner=self.planner, prior=0)

...
class RunAnalyzer(object):
    ...
    def get_run_dataframe(self, directory, agent_name='', gamma=0.95, subsample=20):
        ...
        data.update({
            ...
            "lane_change": [(Counter(episode)[0] + Counter(episode)[2]) for episode in run_data["episode_action"]]
        })
        # try:
        #     dt = 1.0
        #     data.update({
        ...
        df["agent"] = agent_name
        df["lane_change"] = df["lane_change"] / df["length"]

        # Filtering
        for field in ["total reward", "discounted rewards", "length", "crashed", "speed", "lane_change"]:
            try:
